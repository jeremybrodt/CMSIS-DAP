...
static void initDisconnect(uint8_t success) {
    ...
    if (autocrp)
    {
        // first we need to disconnect the usb stack 
        usbd_connect(0);
...
int target_init() {
    ...
    if ((target_initialized != 1) && (DAP_Data.debug_port == DAP_PORT_DISABLED)) {   // Target interface not initialized yet and debug port not used by another application?
        ...
        if (file_type == BIN_FILE || file_type == PAR_FILE ||
            ...
            if ((start_sector != 0) && (start_sector < begin_sector) && (current_sector - (begin_sector - start_sector) >= nb_sector)) {
                ...
                for (i = 0; i < nb_sector_to_move; i++) {
                    if (!target_read_memory(move_sector_start + i*USB_MSC_BUF_SIZE, (uint8_t *)usb_buffer, USB_MSC_BUF_SIZE)) {
                        failSWD();
                        return -1;
                    }
...
void usbd_msc_write_sect (uint32_t block, uint8_t *buf, uint32_t num_of_blocks) {
    ...
    if ((block == SECTORS_ROOT_IDX) || (block == (SECTORS_ROOT_IDX+1))) {
        ...
        if (idx_size != -1) {
            ...
            if ((sector_received_first == 1) && (current_sector == nb_sector) && (target_initialized == 1)) {
            ...
            if ((sector_received_first == 1) && (start_sector == begin_sector) && (current_sector > nb_sector) && (target_initialized == 1)) {
                initDisconnect(1);
                return;
            }
        }
